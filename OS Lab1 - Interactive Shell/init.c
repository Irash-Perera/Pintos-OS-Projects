...
main (void)
{
  ...
  printf("\nWelcome to the interactive shell!");//print welcome message
  while (true) {
    ...
    if(strcmp(input,"whoami")==0){
      printf("\nS.A. Irash Madhuwantha Perera - 210471F");
    }

    else if(strcmp(input,"shutdown")==0){
      printf("\nShutting down...\nHave a nice day!");
      timer_sleep(200); //wait for 200 ticks to read the message
      shutdown_power_off();
    } 
    
    else if (strcmp(input,"time")==0){
      timer_sleep(100); //wait for 100 ticks to do further calculations
      int64_t ticks = timer_ticks();
      int64_t seconds = ticks / TIMER_FREQ;
      printf("\nSeconds since Unix epoch: %d", seconds);
    }
      ...
      printf("\nPriority of the current thread: %d",thread_get_priority());
    }

    else if(strcmp(input,"ram")==0){
      printf("\nAvailable RAM for the os: %d KB",init_ram_pages * PGSIZE / 1024);
    }
